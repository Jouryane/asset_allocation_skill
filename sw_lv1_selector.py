...
def get_index_current_pe(index_code, current_df):
    """
    获取申万行业指数的当前PE值
    :param index_code: 申万行业指数代码（如801880）
    :param current_df: 当前估值数据DataFrame
    :return: 当前PE值
    """
    # current_df中的代码格式是801010.SI，需要匹配
    # 尝试两种格式：801010 和 801010.SI
    ...
    pe_value = index_data.iloc[0]['TTM(滚动)市盈率']
...
def select_best_sw_lv1_indices_by_valuation():
    """
    从申万一级行业指数中选出综合性价比最高的指数
    基于申万行业指数的估值数据
    """
    # 加载配置
    config = load_config()
    ...
    # 获取申万一级行业指数列表
    sw_lv1_indices = config.get('sw_lv1_indices', {})
    
    print(f"\n开始处理 {len(sw_lv1_indices)} 个申万一级行业...")
    print(f"所有行业指数平均涨跌幅: {all_indices_avg_change:.2f}%")
    
    # 计算每个行业的综合性价比得分
    index_valuations = []
    processed_count = 0
    for code, info in sw_lv1_indices.items():
        ...
        if current_pe is None:
            print(f"[{processed_count}/{len(sw_lv1_indices)}] {industry_name} ({code}): 无法获取当前PE")
            continue
        ...
        if not history:
            print(f"[{processed_count}/{len(sw_lv1_indices)}] {industry_name} ({code}): 无法获取历史数据")
            continue
        ...
        if pe_percentile is None:
            print(f"[{processed_count}/{len(sw_lv1_indices)}] {industry_name} ({code}): 无法计算PE百分位")
            continue
        ...
        if price_percentile is None:
            print(f"[{processed_count}/{len(sw_lv1_indices)}] {industry_name} ({code}): 无法计算价格百分位")
            continue
        ...
        if comprehensive_score > 0:
            print(f"[{processed_count}/{len(sw_lv1_indices)}] {industry_name} ({code}): 综合得分={comprehensive_score:.2f}, PE百分位={pe_percentile*100:.1f}%, 价格百分位={price_percentile*100:.1f}%, 估值安全分={safety_score:.3f}, 交易活跃度={trading_activity:.3f}, 价格抗跌={price_resilience:.3f}")
            ...
            print(f"[{processed_count}/{len(sw_lv1_indices)}] {industry_name} ({code}): 综合得分=0 (被筛选)")
...
if __name__ == "__main__":
    ...
    print(f"申万一级行业当前估值: {len(current_df)} 个行业")
    print(f"申万一级行业月度估值: {len(monthly_df)} 条记录")
    ...
    # 执行筛选
    best_indices = select_best_sw_lv1_indices_by_valuation()
    
    if best_indices:
        print("\n申万一级行业指数综合性价比排序（得分从高到低）：")
        print("-" * 140)
        ...
        print("-" * 140)
        print(f"\n筛选结果：{len(best_indices)} 个申万一级行业指数通过四重筛选")
        print("\n说明：")
        print("- 当前PE：申万行业指数的TTM滚动市盈率")
        ...
        print("- 数据来源：申万行业指数估值数据（sw_lv1_index_valuation_current.csv, sw_lv1_index_valuation_daily_10years.csv）")
    else:
        print("未获取到有效的申万一级行业指数估值数据")